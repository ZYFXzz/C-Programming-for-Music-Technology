    ...
    void *userData )
{
    ...
    for( i=0; i<framesPerBuffer; i++ )
    {
        ...
        float sample = 2 *(t*frequency / period) - 1.0;
        sample = lowPassFilter(sample, prev_output, alpha);
        prev_output = sample;
        

        // prev_output = sample;
        *out++ = sample * 0.01f; // Adjust amplitude
...
float highPassFilter(float input, float prev_output, float alpha) {
    return alpha * (input + prev_output);
}
