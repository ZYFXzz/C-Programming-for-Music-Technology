#include <fftw3.h>
#define _USE_MATH_DEFINES // for using constant like M_PI
#include <math.h>
...
#define SPECTRUM_BANDS 64  // Number of frequency bands to display
#define SPECTRUM_HEIGHT 15 // Height of spectrum display
...
// draw FFT spectrum utilities with mel-scale
void calculate_spectrum(double *fft_input, fftw_complex *fft_output, float *spectrum,
    ...
    double freq_min = 20.0;    // Hz
    double freq_max = 20000.0; // Hz
    ...
    for (int i = 0; i < num_bands; i++) {
        ...
        spectrum[i] = (spectrum[i] + 60) / 60;        // Adjust range (-60dB to 0dB)
        ...
        spectrum[i] = pow(spectrum[i], 0.7); // Gamma correction
    }
}
...
static int recordCallback(const void *input, void *output, unsigned long frameCount, const PaStreamCallbackTimeInfo *timeInfo, PaStreamCallbackFlags statusFlags, void *userData) {
    ...
    sf_count_t numWritten = sf_writef_short(data->file, (short *)input, frameCount);
    ...
    for (unsigned long i = 0; i < frameCount && i < FFT_SIZE; i++) {
        data->fft_input[i] = (double)in[i] / 32768.0; // Convert to normalized double
    }
...
static int playbackCallback(const void *input, void *output, unsigned long frameCount, const PaStreamCallbackTimeInfo *timeInfo, PaStreamCallbackFlags statusFlags, void *userData) {
    ...
    sf_count_t numRead = sf_readf_short(data->file, (short *)output, frameCount);
    if (numRead != frameCount) {
        memset((out) + numRead, 0, (frameCount - numRead) * sizeof(short));
        return paComplete;
    }
    // spectrum analysis
    for (unsigned long i = 0; i < frameCount && i < FFT_SIZE; i++) {
        data->fft_input[i] = (double)out[i] / 32768.0; // Convert to normalized double
    }
...
void print_device_info(char mode, const PaDeviceInfo *deviceInfo) {
    ...
    printw("Max input channels: %d\n", deviceInfo->maxInputChannels);
    printw("Max output channels: %d\n", deviceInfo->maxOutputChannels);
    if (mode == 'r')
        printw("\nRecording...\nPress 'Q' to stop.");
    else
        printw("\nPlaying...\nPress 'Q' to stop.");
...
int main(int argc, char *argv[]) {
    ...
    // intialization of libraries utilities
    UserData data;
    ...
    if (strcmp(argv[1], "r") == 0) {
        ...
        // Check if file exists by opening it for reading.
        ...
        if (Pa_OpenDefaultStream(&stream, 1, 0, paInt16, SAMPLE_RATE, FRAMES_PER_BUFFER, recordCallback, &data) != paNoError) {
        ...
        if (Pa_OpenDefaultStream(&stream, 0, data.sfinfo.channels, paInt16, data.sfinfo.samplerate, FRAMES_PER_BUFFER, playbackCallback, &data) != paNoError) {
    ...
    initscr();
    start_color();
    timeout(20); // Faster refresh rate
    noecho();
    ...
    print_device_info(argv[1][0], inputDeviceInfo);

    // start stream
    bool is_playback = (strcmp(argv[1], "p") == 0);
    while (Pa_IsStreamActive(stream) == 1) {
        // For playback mode, draw spectrum
